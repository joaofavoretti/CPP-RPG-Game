...
Color TMX::ColorFromTMX(uint32_t color) {
  tmx_col_bytes res = tmx_col_to_bytes(color);
  return *((Color *)&res);
}

void TMX::DrawLayers(tmx_layer *layers, int posX, int posY, Color tint) {
  while (layers) {
    if (layers->visible) {
...
void TMX::DrawLayer(tmx_layer *layer, int posX, int posY, Color tint) {
  switch (layer->type) {
  case L_GROUP:
...
void TMX::DrawLayerImage(tmx_image *image, int posX, int posY, Color tint) {
...
void TMX::DrawLayerObjects(tmx_object_group *objgr, int posX, int posY,
                           Color tint) {
...
void TMX::DrawPolyline(double offset_x, double offset_y, double **points,
...
void TMX::DrawPolygon(double offset_x, double offset_y, double **points,
                      int points_count, Color color) {
...
void TMX::DrawText(tmx_text *text, Rectangle dest, Color tint) {
...
void TMX::DrawLayerTiles(tmx_layer *layer, int posX, int posY, Color tint) {
...
void TMX::DrawTile(tmx_tile *tile, int posX, int posY, Color tint) {
  Texture *image;
  Rectangle srcRect;
  Rectangle destRect;
...
void *TMX::MemReallocTMX(void *address, size_t len) {
  return MemRealloc(address, (int)len);
}

TMX::TMX(const char *filename) {
...
void TMX::Draw(Vector2 pos, Color tint) {
...
bool TMX::IsColliding(Rectangle rect) {
        ...
        for (int x = 0; x < map->width; x++) {
          ...
          if (gid != 0) {
            ...
            if (tile != NULL) {
              ...
              if (collision != NULL) {
                Rectangle tileRect = {(float)x * map->tile_width * scale,
                                      (float)y * map->tile_height * scale,
                                      (float)map->tile_width * scale,
                                      (float)map->tile_height * scale};
